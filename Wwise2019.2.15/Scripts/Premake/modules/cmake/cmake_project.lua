			...
			if p.fileconfig.hasFileSettings(filecfg) then
				if filecfg.compilebuildoutputs then
					for _, output in ipairs(filecfg.buildoutputs) do
						...
						_p(2, '"${CMAKE_CURRENT_LIST_DIR}/%s"', path.getrelative(prj.location, output))
						-- AUDIOKINETIC END
					end
				end
			elseif rule then
...
function m.files(prj)
	...
	tree.traverse(tr, {
		onleaf = function(node, depth)
			...
			_p(1, '"${CMAKE_CURRENT_LIST_DIR}/%s"', path.getrelative(prj.location, node.abspath))
			-- AUDIOKINETIC END
		end
	})
end
...
function m.generate(prj)
	...
	for cfg in project.eachconfig(prj) do
		...
		_p(2, 'ARCHIVE_OUTPUT_DIRECTORY "${CMAKE_CURRENT_LIST_DIR}/%s"', path.getrelative(prj.location, cfg.buildtarget.directory))
		_p(2, 'LIBRARY_OUTPUT_DIRECTORY "${CMAKE_CURRENT_LIST_DIR}/%s"', path.getrelative(prj.location, cfg.buildtarget.directory))
		_p(2, 'RUNTIME_OUTPUT_DIRECTORY "${CMAKE_CURRENT_LIST_DIR}/%s"', path.getrelative(prj.location, cfg.buildtarget.directory))
		-- AUDIOKINETIC END
		_p(1,')')
		...
		if #cfg.externalincludedirs > 0 then
			...
			for _, includedir in ipairs(cfg.externalincludedirs) do
				-- AUDIOKINETIC START
				--_x(2, '"%s"', includedir)
				_x(2, '"${CMAKE_CURRENT_LIST_DIR}/%s"', path.getrelative(prj.location, includedir))
				-- AUDIOKINETIC END
			end
			_p(1, ')')
		end
		...
		if #cfg.libdirs > 0 then
			...
			for _, libdir in ipairs(cfg.libdirs) do
				-- AUDIOKINETIC START
				--_p(2, '"%s"', libdir)
				_x(2, '"${CMAKE_CURRENT_LIST_DIR}/%s"', path.getrelative(prj.location, libdir))
				-- AUDIOKINETIC END
			end
			_p(1, ')')
		end
